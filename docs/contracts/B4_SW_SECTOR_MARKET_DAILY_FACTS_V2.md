...
level-2 industries. The approved consumers are the V4 human-only market
cognition card and `mainline_divergence_strong_acceptance@v2`. Consumer
migration is a separate stage; this contract does not activate either one.
...
For each of the latest 20 market sessions required by the current consumers,
the publisher joins exact-date
...
- each preceding supporting session covering at least 95%;
...
The future production publisher starts after exact-date stock market facts are
ready, normally before 18:00. It publishes the independent readiness bundle
`v4-market-sector-sw-l2`; it never waits for or falls back to late
`ths_board_daily`.

## Compatibility and audit

The existing THS_L2 v1 rows and `v4-market-sector` readiness remain intact
until their consumers migrate. L2 v2 rows coexist in the same physical table
under a different `sector_level` and source version.
